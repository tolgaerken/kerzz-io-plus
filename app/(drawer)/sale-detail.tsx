...
import { TSale, TSaleRow } from '../../types/dto';

export default function SaleDetailScreen() {
  const { saleData } = useLocalSearchParams();
  const sale: TSale = saleData ? JSON.parse(saleData as string) : null;
  ...
  const renderSaleItems = (items: TSaleRow[], title: string) => {
    ...
    return (
      <ThemedView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <ThemedText style={styles.title}>Satış Detayı</ThemedText>
        </View>
        
        <View style={styles.errorContainer}>
          <ThemedText style={styles.errorText}>Satış bilgisi bulunamadı</ThemedText>
        </View>
      </ThemedView>
    );
  }
    ...
    <ThemedView style={styles.container}>
      ...
      >
        ...
        <View style={styles.infoSection}>
          ...
          <ThemedText style={styles.headerSubtitle}>
            Satış Temsilcisi: {sale.sellerName || 'Belirtilmemiş'}
          </ThemedText>
        </View>

        {/* Products Section */}
        {renderSaleItems(sale.products || [], 'Ürünler')}

        {/* Licenses Section */}
        {renderSaleItems(sale.licances || [], 'Lisanslar')}

        {/* Rentals Section */}
        {renderSaleItems(sale.rentys || [], 'Kiralamalar')}
        ...
        <View style={styles.section}>
          ...
          {sale.discount > 0 && (
            <View style={styles.totalContainer}>
              ...
              <ThemedText style={[styles.totalAmount, { color: colors.error }]}>
                -{formatCurrency(sale.discount, 'tl')}
              </ThemedText>
            </View>
          )}
          {sale.tax > 0 && (
            <View style={styles.totalContainer}>
              ...
              <ThemedText style={styles.totalAmount}>{formatCurrency(sale.tax, 'tl')}</ThemedText>
            </View>
          )}
          <View style={[styles.totalContainer, { borderTopWidth: 2, paddingTop: spacing.medium, marginTop: spacing.small }]}>
            ...
            <ThemedText style={[styles.totalAmount, { fontSize: fontSize.medium, fontWeight: 'bold' }]}>
              {formatCurrency(sale.grandTotal || sale.total || 0, 'tl')}
            </ThemedText>
          </View>
        </View>
